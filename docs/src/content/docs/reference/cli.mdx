...
`--send` starts the workspace exec server, prepares the selected conversation
context, and starts the first turn after Git preparation. `--jump` then opens
that thread in the Codex terminal UI. Used alone on a fresh prepared workspace,
`--jump` starts the exec server and lets the TUI bind the thread with your first
...
`close` stops the workspace exec server and removes the managed worktree while
...
`reopen` restores the exact retained Git binding and thread; its exec server
starts on the next `send` or `jump`.
...
See [Run hooks and guards](../../guides/hooks/) to configure commands,
