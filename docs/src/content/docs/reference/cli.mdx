...
default. Put a repository path before the command to select another
repository:
...
- `create` asks for a missing workspace name and lets you choose a registered
  repository when the current directory is not inside a Git repository;
...
| `coco status [-a] [-t \| --sort name\|state] [-r] [-u] [--json]`   | Show an ordered workspace overview              |
| `coco status [-a] [-t \| --sort name\|state] [-r] [-u] -f`         | Follow that collection view                     |
| `coco status WORKSPACE [-g] [-r] [-u] [--json] [--follow]`          | Show or follow one workspace                     |
...
`-r`, and `-u`. Tree output is a human-readable view and cannot be combined
...
Without a workspace, `status --usage` shows open workspaces in the current
...
mode is intentionally excluded. The current top-level schema version is `11`.
Status JSON may include `runtimeResources`; its optional measurements are a
current sample and are not persisted.
