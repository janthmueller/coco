...
`status --quota` shows remaining account-wide windows such as `5h` and
...
If the current Codex login or version cannot provide limits, CoCo prints an
unavailable quota while preserving the rest of the status view. Reading quota
does not start or load a workspace.
...
mode is intentionally excluded. The current top-level schema version is `13`.
