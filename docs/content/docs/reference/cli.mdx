...
count, aggregate RSS, and CPU use on Linux. `--json` includes available
resource details without requiring `-r`. `--follow` also has the short form
`-f`, so `-fr` follows state and resource use together.
