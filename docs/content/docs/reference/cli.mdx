...
Detailed `status WORKSPACE` also reports the workspace execution process. On
Linux this includes its current descendant count, aggregate RSS, and interval
CPU use. Collection status intentionally omits resource sampling.
...
mode is intentionally excluded. The current top-level schema version is `8`.
