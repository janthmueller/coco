...
| `coco delete [WORKSPACE] [-g] [-t] [-b] [-n] [-y]`                  | Permanently remove a closed workspace record    |
...
Detailed `status WORKSPACE` also reports the workspace exec-server state. On
...
thread until `reopen`. Local tracked, untracked, or ignored files require
...
starts on the next `send` or `jump`. `delete` accepts only closed workspaces
and always deletes their CoCo record. Native thread and branch deletion are
separate opt-ins. `-b` applies only to a branch created by CoCo and still at
its recorded closed commit. Short options compose, so `delete -tby` selects
thread, branch, and non-interactive confirmation.
