...
fn carries_staged_and_unstaged_tracked_changes_without_mutating_the_source() {
    ...
    let snapshot = git
        .snapshot_local_state(&repository, &base, true, false)
        .unwrap();
    let plan = git
        .plan_worktree(
            &repository,
...
fn rejects_untracked_files_and_a_different_dirty_base() {
    ...
    assert!(matches!(
        git.snapshot_local_state(&repository, &base, true, false),
    ...
    assert!(matches!(
        git.snapshot_local_state(&repository, &"0".repeat(40), true, false),
...
fn copies_only_ignored_files_selected_by_worktreeinclude() {
    ...
    let snapshot = git
        .snapshot_local_state(&repository, &base, false, false)
        .unwrap();
    let plan = git
        .plan_worktree(
            &repository,
...
fn carries_only_non_ignored_untracked_files_when_explicitly_requested() {
    ...
    let snapshot = git
        .snapshot_local_state(&repository, &base, true, true)
        .unwrap();
    let plan = git
        .plan_worktree(
            &repository,
...
fn rejects_worktreeinclude_selections_above_the_file_count_limit() {
    ...
    assert!(matches!(
        git.snapshot_local_state(&repository, &base, false, false),
