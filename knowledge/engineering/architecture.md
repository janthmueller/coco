...
tags: [architecture, daemon, cli, mcp, sqlite, git, codex, protocol, hooks]
status: draft
---
...
- Schema v10 adds dedicated signal types, a cursor stream, and bounded signal
  ...
  and [CoCo hooks and guards](hooks.md) for the reaction and policy boundary.
  They retain schema v6's minimal `turn_start` operation ledger and
...
- Codex 0.153.4 first established that an empty fresh thread is not resumable merely
  ...
  or unreleased `codex --worktree` behavior is used. The selected 0.154.0 gate
  confirms the same contract.
...
- Native Codex hooks own in-session lifecycle policy. CoCo post-event hooks own
  only durable asynchronous reactions to accepted signals and committed CoCo
  workspace transitions; hook failure never changes the source operation.
  CoCo guards are a separate synchronous boundary for allowing or denying an
  already checked close or deletion before its first effect. They are not
  durable events and never rewrite the request.
...
- Persist CoCo-owned binding, provisioning, and operation changes atomically.
  ...
  transaction. The implemented hook slice uses a narrow transactional outbox
  only for matching CoCo-owned events. Do not expand it to every native state
  transition or use it as another Codex history mirror.
- Keep the validated hook/guard registry as one atomically replaceable daemon
  snapshot. Event selection and one guard chain each use a coherent cloned
  view; an invalid reload never disrupts the active definitions.
...
schema-v11 bridge still carries legacy columns and tables described below so
...
MCP call auditing. Schema v11's hook outbox is limited to its implemented,
documented events and retention contract; it does not justify a general event
bus.
...
3. Insert a hook event and matching delivery rows only when loaded definitions
   select the committed CoCo-owned fact.
...
normalized native event. An implemented hook delivery commits in the same
transaction as its source signal or final workspace transition and is
acknowledged independently. See [CoCo hooks and guards](hooks.md).
...
`active_turn_id` are now migration-only; schema v11 retains schema v6's send
...
The selected released build is Codex 0.154.0. Its real process behavior proves
...
status validation and avoid notification/hydration races. Schema v11 already
provides at-least-once command delivery for its deliberately small hook event
set; it does not yet expose a general live subscription. If one is justified,
replay plus live handoff must be race-free:
...
the last applicable CoCo event cursor, when one exists; it must not apply
...
- Schema v6 still uses SQLite event sequence as its compatibility replay order.
  The native-first target requires durable order only for CoCo-owned operations
  and schema v11's narrow hook outbox; it never reorders App Server history
...
completed entries remain context rather than the current schema-v11
...
- Freeze handoff, worker MCP registry, Agentgateway, native Windows,
...
- Inventory command help, versioned JSON, stable errors, daemon methods, and
  all shipped schema migrations as compatibility fixtures.
...
model-free 0.154.0 tests exercise native thread lifecycle, real MCP isolation,
profile restoration, fork attribution, durable signal replay, and the native
Codex hook boundary. Process coverage additionally exercises CoCo hook delivery
for signal and workspace lifecycle commits, atomic configuration reload, and
the synchronous pre-effect guard boundary for workspace retirement. These close
the automated core/integration gap for the signal, reaction, and first guard
